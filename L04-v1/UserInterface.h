...
template <typename T>
std::string UserInterface<T>::run(Tree*& pc_tree, std::vector<std::string>& vec_vars, Parser*& parser, bool &b_run) {
	std::string s_out;
	...
	// COMP
		...
		if (pc_tree == NULL) {
			...
			std::pair<std::map<std::string, T>, E_ERROR_TYPE> map_values = mapValuesToVariables<T>(vec_vars, s_input);
			if (map_values.second == E_ERROR_TYPE::INVALID_NUMBER_OF_VALUES) {
				...
				for (auto it = map_values.first.begin(); it != map_values.first.end(); it++) {
					s_out += it->first + " = " + it->second + "\n";
				}
				s_out += "Result: " + Evaluator<T>().dEvaluateTree(pc_tree, map_values.first);
			}
		}
	}
